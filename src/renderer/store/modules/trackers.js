...
export const actions = {
  updateTrackings ({ state, commit, dispatch, getters }) {
    ...
    prevs.filter(id => !currents.includes(id))
      .forEach(id => { dispatch('stopActivity', id) })

    // Start activity when process was started.
    currents.filter(id => !prevs.includes(id))
      .forEach(id => { dispatch('startActivity', id) })
  ...
  stopActivity ({ dispatch, rootGetters }, projectId) {
    ...
    if (activity) {
      dispatch('activities/updateActivity', {
        id: activity.id,
        stoppedAt: `${new Date()}`
      }, { root: true })
    }
  ...
  deleteTracker ({ commit, dispatch }, payload) {
    ...
    dispatch('entities/deleteEntitiy',
...
export const getters = {
  ...
  getWorkingProjectIds (state, getters, rootState, rootGetters) {
    return uniq(
      getters.getTrackers.filter(tracker =>
